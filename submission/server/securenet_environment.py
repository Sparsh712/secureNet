...
class SecureNetEnvironment:
    REWARDS = {
        "step_penalty":      -0.02,   # reduced — less punishing per step
        "query_logs_new":    +0.10,   # first time querying a node
        "query_logs_repeat": -0.02,   # penalty for spamming same node
        "query_infected":    +0.25,   # discovering an infected node's logs
        "analyze_hit":       +0.40,   # raised — finding malicious process
        "analyze_miss":      -0.02,   # tiny penalty for wrong node
        "threat_intel_hit":  +0.25,
        "threat_intel_miss": -0.01,
        "block_ip":          +0.50,
        "block_ip_repeat":   -0.05,   # already blocked
        "quarantine_hit":    +0.60,
        "quarantine_miss":   -0.10,
        "isolate_infected":  +1.00,
        "isolate_healthy":   -0.80,
        "catastrophic":      -3.00,
    }
    ...
    def reset(self, difficulty: str = None) -> Dict[str, Any]:
        ...
        self.blocked_ips  = []
        self.queried_nodes = set()  # tracks already-queried nodes
        self.analyzed_nodes = set() # tracks already-analyzed nodes
    ...
    def _do_query_logs(self, node, timeframe, base):
        ...
        if node in self.queried_nodes:
            # Already queried — small penalty to discourage spamming
            r = base + self.REWARDS["query_logs_repeat"]
        else:
            self.queried_nodes.add(node)
            r = base + self.REWARDS["query_logs_new"]
            # Bonus for discovering an infected node's logs for the first time
            if n["status"] == "infected":
                r += self.REWARDS["query_infected"]
    ...
    def _do_analyze(self, node, base):
        ...
        already = node in self.analyzed_nodes
        self.analyzed_nodes.add(node)
        if sus:
            # First-time hit gets full reward; repeat analysis still valuable
            r = base + self.REWARDS["analyze_hit"] * (1.0 if not already else 0.3)
            res = (
            ...
            r = base + self.REWARDS["analyze_miss"] * (1.0 if not already else 2.0)  # double penalty for re-checking clean
            res = (
                f"PROCESS ANALYSIS — {node}:\n"
    ...
    def _do_block_ip(self, ip, base):
        ...
        if ip in self.blocked_ips:
            return f"IP {ip} already blocked.", base + self.REWARDS["block_ip_repeat"]
        ...
        if ip in att:
            ...
            r   = base - 0.05   # minor penalty — non-IOC IP blocked
    ...
    def _grade_episode(self, message: str, last_reward: float) -> Dict[str, Any]:
        ...
        # Time bonus: step-count based (walls clock is meaningless in RL training)
        # Faster resolution within step budget = higher bonus (up to +0.15)
        step_frac  = self.step_count / max(self.max_steps, 1)
        time_bonus = max(0.0, 0.15 * (1.0 - step_frac))  # up to +0.15 for fastest solve

        # Efficiency bonus: reward querying all infected nodes before isolating
        infected_queried = len(set(self._tmpl["compromised"]) & self.queried_nodes)
        coverage_bonus   = 0.05 * (infected_queried / max(len(original), 1))

        score = max(0.0, containment * 0.65 + kc_score * 0.20 + time_bonus + coverage_bonus - fp_pen * 0.5)
        if catastrophic:
            score = 0.0
